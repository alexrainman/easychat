    ...
    public class EasyChatService
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            while (true)
            {
                ...
                //await context.Response.WriteAsync($"Received message - {message} at {DateTime.Now}");

                string function = "alert('{0}');";
                string log = string.Format(string.Format("<script type=\"\" language=\"\">{0}</script>", function), message);
